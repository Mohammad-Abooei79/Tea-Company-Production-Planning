...
for i in range(10):
    Model.CR[0] = Model.CR[0] + 100
    Solver = SolverFactory("cplex")
    Solver.solve(Model)
    X.append(value(Model.CR[0]))
    Y.append(value(Model.objective)/pow(10,9))
    print(value(Model.CR[0]), ':',value(Model.objective))
    print("---------------------")
'''    for k in Model.k:
        print(value(Model.Y[k,1])) '''

plt.plot(X , Y)
plt.title("Sensitivity Analysis on production cost of week 1")
