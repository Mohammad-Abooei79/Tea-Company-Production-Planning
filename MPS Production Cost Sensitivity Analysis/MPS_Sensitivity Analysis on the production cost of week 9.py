...
for i in range(10):
    Model.CR[8] = Model.CR[8] + 100
    Solver = SolverFactory("cplex")
    Solver.solve(Model)
    X.append(value(Model.CR[8]))
    Y.append(value(Model.objective)/pow(10,9))
    print(value(Model.CR[8]), ':',value(Model.objective))
    print("---------------------")
'''    for k in Model.k:
        print(value(Model.Y[k,9])) '''

plt.plot(X , Y)
plt.title("Sensitivity Analysis on production cost of week 9")
