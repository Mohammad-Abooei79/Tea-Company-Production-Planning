...
Model.D = Param(Model.s , initialize = Ds , mutable = True, within = NonNegativeReals)

Model.H = Param(Model.s , initialize = Dh , mutable = True, within = NonNegativeReals)
#Variables
...
for i in range(10):
    Model.D[0] = Model.D[0] + 500
    Solver = SolverFactory("cplex")
    Solver.solve(Model)
    X.append(value(Model.D[0]))
    Y.append(value(Model.objective)/pow(10,9))
    print(value(Model.D[0]), ':',value(Model.objective))
    print("---------------------")

plt.plot(X , Y)
plt.title("Sensitivity Analysis on demand of Period 1")
plt.xlabel("Demand")
