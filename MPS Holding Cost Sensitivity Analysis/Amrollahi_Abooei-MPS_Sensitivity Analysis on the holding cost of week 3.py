...
for i in range(10):
    Model.H[2] = Model.H[2] + 100
    Solver = SolverFactory("cplex")
    Solver.solve(Model)
    X.append(value(Model.H[2]))
    Y.append(value(Model.objective)/pow(10,9))
    print(value(Model.H[2]), ':',value(Model.objective))
    print("---------------------")
'''    print(value(Model.I[1,3]), value(Model.I[2,3]), value(Model.I[3,3]), value(Model.I[4,3])
          , value(Model.I[5,3]))'''

plt.plot(X , Y)
plt.title("Sensitivity Analysis on holding cost of week 3")
