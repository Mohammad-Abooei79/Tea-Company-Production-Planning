...
for i in range(10):
    Model.H[6] = Model.H[6] + 100
    Solver = SolverFactory("cplex")
    Solver.solve(Model)
    X.append(value(Model.H[6]))
    Y.append(value(Model.objective)/pow(10,9))
    print(value(Model.H[6]), ':',value(Model.objective))
    print("---------------------")
'''    print(value(Model.I[1,7]), value(Model.I[2,7]), value(Model.I[3,7]), value(Model.I[4,7])
              , value(Model.I[5,7]))'''

'''plt.xlim(2600, 3700)
plt.ylim(22.145381250, 22.147743750)
plt.autoscale(False)'''
plt.plot(X , Y)
plt.title("Sensitivity Analysis on holding cost of week 7")
